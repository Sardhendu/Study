...
LIKELIHOOD: say the response variable comes from a normal distribution with mean (mu) and variance (prec)
  # b[1] is the intercept term and b[2] is the coefficient for the explanatory variable income and b[3] is oil indicator
  # Note we do it in log scale

PRIOR, The priors are assigned to the variables b[1] and b[2], b[3].
  # We assume a non-informative prior, where we say tha the prior takes from a normal distribution with   # infinite variance, which makes it a flat distribution. Note: Unlike flat uniform distribution,
...
# Going back to out lessons: We know when the mean (mu) of y is known then the prior for the  
  ...
  # mu = b0 + b1x, lets take variance from Inv-Gamma.
