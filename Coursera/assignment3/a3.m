...
function test_gradient(model, data, wd_coefficient)
  disp('Inside the test_gradient function')
  base_theta = model_to_theta(model);
  disp(size(base_theta(0)))
  h = 1e-2;
...
disp('Inside the ret-model_to_theta function')
  ...
  disp('The shape of input_to_hid_transpose is: '),disp(size(input_to_hid_transpose))
  disp ('The shape of hid_to_class_transpose is :'),disp(size(hid_to_class_transpose))
