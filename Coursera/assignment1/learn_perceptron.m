...
fprintf('The negative examples are: ');
disp(neg_examples)
fprintf('The positive examples are: ');
disp(pos_examples)
...
if (~exist('w_init','var') || isempty(w_init))
    fprintf('Adding Random Weight');
    w = randn(3,1);
else
    fprintf('The weights are provided \n');
    disp(w_init)
    w = w_init;
end

if (~exist('w_gen_feas','var'))
    fprintf('The general feasible weights are present');
    w_gen_feas = [];
end
...
fprintf('Indices of Negative error: ');
disp(mistakes0);
fprintf('Indices of Positive error: ');
disp(mistakes1);
...
while (num_errs > 0)
    ...
    %w = update_weights1(neg_examples,pos_examples,w);
...
for i=1:num_neg_examples
    ...
    if (activation >= 0)
        fprintf('Hey!! Dude I want you to update the code, when negative mislassified \n');
        fprintf('The activation for negative example is: %d', activation);
        %YOUR CODE HERE
        fprintf(['weights:\t', mat2str(w), '\n']);
        fprintf(['x:\t', mat2str(x), '\n']);
        w = w-x;
...
for i=1:num_pos_examples
    ...
    activation = this_case*w;
    if (activation < 0)
        fprintf('Hey!! Dude I want you to update the code when positive misclassified \n');
        %fprintf('The activation for negative example is: %d', activation);
        %YOUR CODE HERE
        %disp(w);
        w = w+x;
...
function [w] = update_weights1(neg_examples, pos_examples, w_current)
%% 
% Updates the weights of the perceptron for incorrectly classified points
% using the perceptron update algorithm. This function makes one sweep
% over the dataset.
...
for i=1:num_neg_examples
    x = neg_examples(i,:)';
    %fprintf('The current negative training set running is: \n');
    %disp(x);
    activation = x'*w;
    if (activation >= 0)
...
for i=1:num_pos_examples
    %fprintf('The current positive training set running is: \n');
    %disp(x);
    x = pos_examples(i,:)';
    activation = x'*w;
    if (activation < 0)
...
% addpath "/Users/sam/All-Program/App-Dataset/Neural-Nets-Coursera"
% addpath "/Users/sam/All-Program/App/Neural-Nets-Coursera"
% load dataset1
% w = learn_perceptron(neg_examples_nobias,pos_examples_nobias,w_init,w_gen_feas)



% ans 1 = dataset2, dataset4
% ans 2 = No
